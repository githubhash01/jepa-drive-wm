"""
Depth metrics, copied from the DINOv3 depth eval
(probes/depth/dinov3_depth/metrics.py) to keep this package self-contained.

``calculate_depth_metrics`` returns a namedtuple of the standard monocular-depth
metrics (a1/a2/a3, abs_rel, rmse, ...). Pass a boolean ``valid_mask``; it is further
restricted to ``gt > 0`` internally.
"""
...
_DepthMetricValues = namedtuple("DepthMetricValues", [metric.name for metric in DEPTH_METRICS])
